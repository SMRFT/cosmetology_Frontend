...
const ProcedureComponent = () => {
    ...
    const fetchProcedures = async (date) => {
        try {
            ...
            const response = await axios.get(`http://127.0.0.1:8000/get_procedures_bill/?appointmentDate=${formattedDate}&branch_code=${branchCode}`);
    ...
    const handleDownload = () => {
        ...
        convertToBase64(PDFHeader, headerImage => {
            convertToBase64(PDFFooter, footerImage => {
                ...
                doc.text(`Branch: ${branchCode}`, 140, textYOffset + 6);
