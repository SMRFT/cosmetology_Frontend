...
const ProcedureComponent = () => {
    ...
    const fetchProcedures = async (date) => {
        try {
            ...
            const response = await axios.get(`https://api.shinovadatabase.in/get_procedures_bill/?appointmentDate=${formattedDate}`);
    ...
    const handleInputChange = (inputValue, index) => {
        const newRecords = [...consumerRecords];
        newRecords[index].item = inputValue;
        setConsumerRecords(newRecords);
    };
    ...
    const handleSave = async () => {
        if (selectedPatient) {
            ...
            try {
                const response = await axios.post('https://api.shinovadatabase.in/Post_Procedure_Bill/', payload);
  ...
  const handleDownload = () => {
    ...
    convertToBase64(PDFHeader, headerImage => {
      convertToBase64(PDFFooter, footerImage => {
        ...
        const now = new Date();
        const formattedAppointmentDate = new Date(selectedPatient.appointmentDate).toLocaleDateString(); // Format as needed
        // const formattedAppointmentTime = new Date(selectedPatient.appointmentDate).toLocaleTimeString(); // If time is also needed
        
        doc.text(`Date: ${formattedAppointmentDate}`, 140, textYOffset);
        // doc.text(`Time: ${formattedAppointmentTime}`, 140, textYOffset + 6);
