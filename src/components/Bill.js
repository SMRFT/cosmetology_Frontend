...
import PDFMain from './images/PDF_Main.jpeg';
...
const Bill = () => {
  ...
  const fetchPatientData = async (date) => {
    try {
      const response = await fetch(`https://api.shinovadatabase.in/summary/post/patient_details/?appointmentDate=${format(date, 'yyyy-MM-dd')}`);
      if (!response.ok) {
  ...
  const fetchBillingData = async (date) => {
    try {
      const response = await fetch(`https://api.shinovadatabase.in/summary/post/?appointmentDate=${format(date, 'yyyy-MM-dd')}`);
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
  ...
  const fetchMedicineDetails = async (medicine_name) => {
    try {
      const response = await fetch(`https://api.shinovadatabase.in/medicine_name/data/?medicine_name=${encodeURIComponent(medicine_name)}`);
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      const data = await response.json();
    ...
    .flatMap((item, itemIndex) => {
      ...
      return validPrescriptions.map((prescription, prescriptionIndex) => {
        ...
        return {
          ...
          qty,
          price,
          total, // This will now be a numeric value or '0.00'
...
const updateStock = async () => {
  ...
  for (const stockUpdate of stockUpdates) {
    try {
      const response = await fetch('https://api.shinovadatabase.in/update_stock/', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
