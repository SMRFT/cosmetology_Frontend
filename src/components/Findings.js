...
const Findings = ({ preSelectedFindings, onSelectFindings}) => {
  ...
  useEffect(() => {
    axios.get('https://api.shinovadatabase.in/Findings/')
      .then(response => {
        setFindingsList(response.data);
      })
      .catch(error => {
        console.error('Error fetching findings data:', error);
        toast.error('Error fetching findings data');
      });
  }, []);
  ...
  const handleAddNewFinding = () => {
    axios.post('https://api.shinovadatabase.in/Findings/', { findings: newFinding })
      .then(response => {
      ...
      toast.error('Error adding new finding'); // Error toast for adding
