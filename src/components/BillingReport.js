...
const BillingReport = () => {
  ...
  const fetchData = async (interval) => {
    ...
    try {
      // Updated to include branch_code in the request
      const response = await axios.get(`http://127.0.0.1:8000/billing/${interval}/?appointmentDate=${dateParam}&branch_code=${branchCode}`, {
        headers: {
  ...
  const downloadCSV = () => {
    ...
    link.href = url;
    link.setAttribute('download', `${getReportHeading(selectedInterval)}_${branchCode}.csv`); // Added branch_code to filename
  ...
  useEffect(() => {
    fetchMedicineData();
  }, [branchCode]); // Added branchCode as dependency

  // Function to fetch the price of the selected medicine
  const fetchMedicinePrice = async (medicine_name) => {
    try {
      // Updated to include branch_code in the request
      const response = await axios.get(`http://127.0.0.1:8000/pharmacy/medicine/${medicine_name}/price/?branch_code=${branchCode}`, {
        headers: {
          'X-Branch-Code': branchCode
        },
        withCredentials: true
